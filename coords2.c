...
main(int argc,char*argv[])
{
	...
	int i,nw,nr,fd,j =0;
	int buffer [4096*sizeof(Coord)];
	
	Mylist *coordlist=malloc(sizeof(Coord));
	initlist(coordlist);

	if (argc == 1){	//Leer e imprimir en orden inverso//
		printf("Para introducir coordenas pulse primero un espacio:\n");
		char str[24];
  		int c,n; 
  		i=0;
  		n=0;
  		...
  		Coord *coord_aux=malloc(sizeof(Coord));
    		for (i=0; i < sizeof str && (c=getchar())!=EOF && c!='\n'; i++){
				str[i] = (c-48); 
					if (c!=32){
							n = n+1;
					}
					if ((n % 2 != 0) &&(c!=32)) {
						printf("entro en impar\n");
						coord_aux->x = str[i];
					}
					if ((n % 2 == 0 ) &&(c!=32)){
						coord_aux->y = str[i];
